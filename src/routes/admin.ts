...
import { getDb, DB_PATH, replaceDb, RESTORE_DIR } from '../db';
...
router.get('/stats', (_req: Request, res: Response) => {
  const db = getDb();
  const userCount = (db.prepare('SELECT COUNT(*) as c FROM users WHERE is_admin=0').get() as any).c;
...
router.get('/users', (_req: Request, res: Response) => {
  const users = getDb().prepare(
    'SELECT id,username,must_change_password,created_at FROM users WHERE is_admin=0 ORDER BY username'
  ).all();
  res.json({ users });
});
...
router.post('/users/:id/reset-password', validateCsrf, async (req: Request, res: Response) => {
  ...
  const tempPassword = crypto.randomBytes(5).toString('hex') + '!';
  const hash = await bcrypt.hash(tempPassword, 12);
  getDb().prepare('UPDATE users SET password_hash=?,must_change_password=1 WHERE id=?').run(hash, userId);
